...
else
{
	...
	else
	{
		if($arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"])
		{
			...
			?>
			<div id="form_comment_" style="display:none;" class = "reviews__own">
				<article class="review-form" id="form_c_del" style="display:none;">
				<form method="POST" class = "form js-form-validate" name="form_comment" id="<?=$component->createPostFormId()?>" action="<?=$ajaxPath; ?>">
				...
				echo bitrix_sessid_post();?>
					...
					if(empty($arResult["User"]))
					{
						...
						<div class="form__row">
							...
							<div class="form__col form__col_width_260">
								<input  value="<?=htmlspecialcharsEx($_SESSION["blog_user_name"])?>" id="user_name" name="user_name" required="" class="input" type="text" aria-required="true">
							</div>
						</div>
						...
						<div class="form__row">
							...
							<div class="form__col form__col_width_260">
								<input value="<?=htmlspecialcharsEx($_SESSION["blog_user_email"])?>" id="user-email" name="user_email" type="email" required="" class="input" aria-required="true">
							</div>
						</div>
					...
					<div class="form__row review-form__antispam-row">
													<div class="form__col form__col_width_130">
														<label for="captcha_word" class="form__label">Защита от спама</label>
													</div>
													<div class="form__col form__col_width_130">
														<input type="hidden" name="captcha_code" id="captcha_code" value="<?=$arResult["CaptchaCode"]?>">
														<input id="captcha_word" name="captcha_word" required="" class="input" type="text" aria-required="true"><a href="javascript:void(0);" onclick="updateCaptcha()" class="link review-form__refresh-captcha text">Обновить картинку</a>
													</div>
													<div class="form__col form__col_width_130" id="div_captcha">
														<img  id="captcha" src="assets/images/capcha.jpg" alt="защита от спама">
													</div>
												</div>
...
//					only for not registered users
					...
					<div class="form__row review-form__btn-row">
													<div class="form__col form__col_width_280">
														<button type="button"   name="sub-post" id="post-button"  class="btn review-form__submit" onclick="submitCommentNew()">Оставить отзыв</button>
													</div>
...
<article class="review reviews__item">
											<div class="raiting review__raiting"><span class="raiting__star">5 звезда</span><span class="raiting__star raiting__star_active">4 звезды</span><span class="raiting__star">3 звезды</span><span class="raiting__star">2 звезды</span><span class="raiting__star">1 звёзд</span>
											</div>
											...
											<footer class="review__footer">
												<p>Добавил(а) <?= $comment["AuthorName"] ?>,
													<time datetime="2015-03-15T21:07"><?= $comment["DateFormated"] ?></time>
												</p>
											</footer>
										</article>

				<a name="<?=$comment["ID"]?>"></a>
				<article class="review reviews__item" style="padding-left:<?=$paddingSize?>em;">
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div id="record-<?=$arParams["ENTITY_XML_ID"]?>-<?=$comment["ID"]?>-placeholder" class="reviews__own">
			...
			if($arResult["CanUserComment"])
			{
				...
				<div id="form_comment_0">
					...
					<div class="reviews__own">
						<article id="record-<?=$arParams["ENTITY_XML_ID"]?>-0-placeholder" class="review-form" >
